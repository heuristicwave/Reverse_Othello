...
class Othello:
    def __init__(self, ip, port):
        ...
        if code != 'accept':
            self.log.write('ERROR: accept msg error: ' +
                           code + ' ' + str(data))
            sys.exit(-1)
        ...
        self.log.write('Color: {color}, Token: {token}, Board: {board}'.format(
            color=self.color, token=self.token, board=self.board))

    def wait_for_turn(self):
        print("it's wait for turn")
        while True:
            ...
            if code == 'turn':
                ...
                self.log.write(
                    "game end: " + data['status'] + " " + data['score'])
                self.log.write('final board: ' + data['board'])
                return code, data
            else:
                self.log.write('ERROR: invaild code error: ' +
                               code + ' ' + str(data))
                sys.exit(-1)
